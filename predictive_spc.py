...
Run:
    streamlit run predictive_spc.py
"""
...
def get_base_numeric_features(feature_cols: List[str]) -> List[str]:
    """
    Only allow direct numeric process/engineered features for relationship analysis.
    Exclude one-hot encoded categorical flags.
    """
...
def conditional_feature_effect_curve(
    ...
    """
    Estimate feature effect while partially respecting feature dependence.

    Method:
    1. Find rows in the dataset that are similar to the current row on other numeric features
    2. Use the selected feature values from those nearby rows as realistic candidate values
    3. Replace only the selected feature in the current row and score risk
    4. Return candidate values and predicted risk

    This is more dependence-aware than a raw global sweep.
    """
    ...
    if not compare_features:
        ...
        stds = compare_matrix.std(ddof=0).replace(0, 1.0)

        z_work = (compare_matrix - means) / stds
        z_current = (current_compare - means) / stds
...
def summarize_feature_relationship(curve_df: pd.DataFrame, feature_name: str) -> str:
    """
    Plain-English relationship summary based on the conditional effect curve.
    """
    ...
    if np.isfinite(corr) and corr >= 0.35 and slope > 0:
        return (
    ...
    elif np.isfinite(corr) and corr <= -0.35 and slope < 0:
        return (
...
with tab1:
    ...
    if curve_df.empty:
        ...
        current_feature_value = float(current_row[selected_feature])
        current_pred = float(
