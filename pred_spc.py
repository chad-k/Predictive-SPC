...
def make_demo_data(
    ...
    for p_idx, part in enumerate(parts):
        ...
        for m_idx, machine in enumerate(machines):
            ...
            timestamps = pd.date_range(
                start=start_ts, periods=rows_per_combo, freq=f"{freq_minutes}min"
            )
    ...
    df = pd.DataFrame(records).sort_values(["part_no", "machine", "timestamp"]).reset_index(drop=True)
    return df
...
def add_features_and_targets(df: pd.DataFrame, future_horizon: int = 8) -> pd.DataFrame:
    ...
    for win in [3, 5, 8, 12, 20]:
        out[f"roll_mean_{win}"] = grp["measurement"].transform(lambda s: s.rolling(win, min_periods=2).mean())
        out[f"roll_std_{win}"] = grp["measurement"].transform(lambda s: s.rolling(win, min_periods=2).std(ddof=0))
        out[f"roll_min_{win}"] = grp["measurement"].transform(lambda s: s.rolling(win, min_periods=2).min())
        out[f"roll_max_{win}"] = grp["measurement"].transform(lambda s: s.rolling(win, min_periods=2).max())
    ...
    out["baseline_center_20"] = grp["measurement"].transform(lambda s: s.rolling(20, min_periods=8).mean())
    out["baseline_sigma_20"] = grp["measurement"].transform(lambda s: s.rolling(20, min_periods=8).std(ddof=0))
    ...
    out["run_same_side_8"] = grp["z_from_baseline"].transform(
        lambda s: s.rolling(8, min_periods=3).apply(lambda x: consecutive_same_side_run(list(x)), raw=False)
    )
    out["run_trend_8"] = grp["measurement"].transform(
        lambda s: s.rolling(8, min_periods=3).apply(lambda x: consecutive_trend_run(list(x)), raw=False)
    )

    out["is_oos_now"] = (
        ((out["measurement"] < out["lsl"]) | (out["measurement"] > out["usl"])).astype(int)
    )
    out["near_spec_warn"] = ((out["pct_of_spec_used"] >= 0.80) & (out["pct_of_spec_used"] < 1.0)).astype(int)
    ...
    for _, g in original_groups:
        ...
        future_oos = np.full(len(g), np.nan)
        future_rule = np.full(len(g), np.nan)
        ...
        for i in range(len(g)):
            ...
            future_oos[i] = int(
                ((future_slice["measurement"] < future_slice["lsl"]) | (future_slice["measurement"] > future_slice["usl"])).any()
            )
            ...
            if pd.isna(c) or pd.isna(s) or s <= 1e-12:
                ...
                future_rule[i] = nelson_like_rule_trigger(future_slice["measurement"], c, s)
...
def make_spc_chart(df_plot: pd.DataFrame, selected_idx: int) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        x=df_plot["timestamp"], y=df_plot["measurement"], mode="lines+markers", name="Measurement"
    ))
    fig.add_trace(go.Scatter(
        x=df_plot["timestamp"], y=df_plot["target"], mode="lines", name="Target"
    ))
    fig.add_trace(go.Scatter(
        x=df_plot["timestamp"], y=df_plot["usl"], mode="lines", name="USL"
    ))
    fig.add_trace(go.Scatter(
        x=df_plot["timestamp"], y=df_plot["lsl"], mode="lines", name="LSL"
    ))
    fig.add_trace(go.Scatter(
        x=df_plot["timestamp"], y=ucl, mode="lines", name="Rolling UCL"
    ))
    fig.add_trace(go.Scatter(
        x=df_plot["timestamp"], y=lcl, mode="lines", name="Rolling LCL"
    ))
...
def make_risk_chart(df_plot: pd.DataFrame) -> go.Figure:
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=df_plot["timestamp"], y=df_plot["pred_prob_oos"], mode="lines", name="Future OOS Risk"
    ))
    fig.add_trace(go.Scatter(
        x=df_plot["timestamp"], y=df_plot["pred_prob_rule"], mode="lines", name="Future Rule Risk"
    ))
    fig.update_layout(
...
def get_demo_modeling_data(n_parts: int, n_machines: int, rows_per_combo: int, future_horizon: int) -> Tuple[pd.DataFrame, pd.DataFrame]:
    raw = make_demo_data(
        n_parts=n_parts,
...
view_df = scored_df[(scored_df[part_col] == 1) & (scored_df[machine_col] == 1)].sort_values("timestamp").reset_index(drop=True)

if view_df.empty:
    st.error("No data available for that selection.")
    st.stop()

selected_idx = st.slider("Choose current sample index", min_value=0, max_value=len(view_df) - 1, value=len(view_df) - 1)
...
with st.expander("Global Feature Importance"):
    ...
    with gi1:
        ...
        st.dataframe(top_feature_table(bundle.model_oos, bundle.feature_cols), use_container_width=True, hide_index=True)
    with gi2:
        st.markdown("**Global Importance - Rule Model**")
        st.dataframe(top_feature_table(bundle.model_rule, bundle.feature_cols), use_container_width=True, hide_index=True)
...
with st.expander("Model Performance (Demo Data Test Set)"):
    ...
    m2.metric("OOS ROC AUC", f"{bundle.test_metrics['oos_auc']:.3f}" if pd.notna(bundle.test_metrics['oos_auc']) else "N/A")
    m3.metric("Rule Accuracy", f"{bundle.test_metrics['rule_accuracy']:.3f}")
    m4.metric("Rule ROC AUC", f"{bundle.test_metrics['rule_auc']:.3f}" if pd.notna(bundle.test_metrics['rule_auc']) else "N/A")
...
with st.expander("Preview Scored Data"):
    preview_cols = [
        "timestamp", "measurement", "target", "lsl", "usl",
        "temperature", "pressure", "speed", "humidity", "wear_index",
        "pred_prob_oos", "pred_prob_rule", "target_future_oos", "target_future_rule",
        "pct_of_spec_used", "slope_8", "recent_oos_8", "recent_warn_8",
    ]
...
- uploaded CSV or Excel
- SQL Server, historian, or OPC data
- real subgroup measurements
- actual Nelson rule engine and cavity-aware logic
"""
    )